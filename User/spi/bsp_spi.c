...
uint8_t qst_fis210x_spi_write(uint8_t Addr, uint8_t Data)
{
	SPI_QST_CS_LOW();
	spi_delay(1);
	Spi_SendByte(Addr&0x3f);		// write commond
	Spi_SendByte(Data);
	spi_delay(1);
	SPI_QST_CS_HIGH();
...
uint8_t qst_fis210x_spi_write_bytes(uint8_t Addr, uint8_t* Data, uint8_t Len)
{
	uint8_t i;

	for(i=0; i<Len; i++)
	{
		qst_fis210x_spi_write(Addr+i, Data[i]);
	}
...
uint8_t qst_fis210x_spi_read(uint8_t Addr, uint8_t *pData, uint16_t Length)
{
	...
	if(Length == 1)
		...
		Spi_SendByte(Addr|0xc0);
	for(i =0; i < Length; i++){
	  pData[i] = Spi_SendByte(0x00);
	}  
	spi_delay(1);
	SPI_QST_CS_HIGH();
...
unsigned char qmp6988_spi_write(unsigned char Addr, unsigned char Data)
{
	SPI_QST_CS_LOW();
	spi_delay(20);
	Spi_SendByte(Addr&0x7f);		// write commond
	Spi_SendByte(Data);
	spi_delay(20);
	SPI_QST_CS_HIGH();
...
unsigned char qmp6988_spi_read(unsigned char Addr, unsigned char *pData, unsigned char Length)
{
	unsigned char i = 0;
