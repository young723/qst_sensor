...
void FisImu_read_acc_xyz(float acc_xyz[3])
{
	...
	FisImu_read_reg(FisRegister_Ax_L|0x80, buf_reg, 6); 	// 0x19, 25
#endif
...
void FisImu_read_gyro_xyz(float gyro_xyz[3])
{
...
#if defined(QST_USE_SPI)
	FisImu_read_reg(FisRegister_Gx_L, &buf_reg[0], 6); 	// 0x1f, 31
#else
	...
	FisImu_read_reg(FisRegister_Gx_L|0x80, buf_reg, 6);  	// 0x1f, 31
#endif
...
void FisImu_read_xyz(float acc[3], float gyro[3], unsigned char *tim_count)
{
	...
	//FisImu_read_reg(FisRegister_Gx_L, &buf_reg[6], 1);	
	//FisImu_read_reg(FisRegister_Gx_H, &buf_reg[7], 1);
	//FisImu_read_reg(FisRegister_Gy_L, &buf_reg[8], 1);
	//FisImu_read_reg(FisRegister_Gy_H, &buf_reg[9], 1);
	//FisImu_read_reg(FisRegister_Gz_L, &buf_reg[10], 1);
	//FisImu_read_reg(FisRegister_Gz_H, &buf_reg[11], 1);
...
#if defined(FISIMU_UINT_MG_DPS)
	// mg
	acc_t[AXIS_X] = (float)(raw_acc_xyz[AXIS_X]*1000.0f)/acc_lsb_div;
	acc_t[AXIS_Y] = (float)(raw_acc_xyz[AXIS_Y]*1000.0f)/acc_lsb_div;
	acc_t[AXIS_Z] = (float)(raw_acc_xyz[AXIS_Z]*1000.0f)/acc_lsb_div;
#else
	// m/s2
...
#if defined(FISIMU_UINT_MG_DPS)
	// dps
	gyro_t[0] = (float)(raw_gyro_xyz[0]*1.0f)/gyro_lsb_div;
	gyro_t[1] = (float)(raw_gyro_xyz[1]*1.0f)/gyro_lsb_div;
	gyro_t[2] = (float)(raw_gyro_xyz[2]*1.0f)/gyro_lsb_div;
#else
	// rad/s
	gyro_t[AXIS_X] = (float)(raw_gyro_xyz[AXIS_X]*0.01745f)/gyro_lsb_div;		// *pi/180
	gyro_t[AXIS_Y] = (float)(raw_gyro_xyz[AXIS_Y]*0.01745f)/gyro_lsb_div;
	gyro_t[AXIS_Z] = (float)(raw_gyro_xyz[AXIS_Z]*0.01745f)/gyro_lsb_div;
#endif	
...
void FisImu_read_xyz_raw(short raw_acc_xyz[3], short raw_gyro_xyz[3], unsigned char *tim_count)
{
	unsigned char	buf_reg[12];
...
uint8_t FisImu_init(void)
{
	...
	if(chip_id == 0xfc)
	{
		...
		fisimu_config.accOdr = AccOdr_256Hz;
		fisimu_config.gyrRange = GyrRange_1024dps;
		fisimu_config.gyrOdr = GyrOdr_256Hz;
		fisimu_config.magOdr = MagOdr_32Hz;
		fisimu_config.magDev = MagDev_AK8963;
		fisimu_config.aeOdr = AeOdr_32Hz;
	
		//FisImu_config_acc(AccRange_8g, AccOdr_128Hz, Lpf_Enable, Hpf_Disable);
		//FisImu_config_gyro(GyrRange_1024dps, GyrOdr_256Hz, Lpf_Enable, Hpf_Disable);
		//FisImu_write_reg(FisRegister_Ctrl7, 0x03);
		FisImu_Config_apply(&fisimu_config);
		if(1)
		{
			uint8_t read_data = 0x00;
			FisImu_read_reg(FisRegister_Ctrl2, &read_data, 1);
			qst_printf("FisRegister_Ctrl2=0x%x \n", read_data);
			FisImu_read_reg(FisRegister_Ctrl3, &read_data, 1);
			qst_printf("FisRegister_Ctrl3=0x%x \n", read_data);
			FisImu_read_reg(FisRegister_Ctrl4, &read_data, 1);
			qst_printf("FisRegister_Ctrl4=0x%x \n", read_data);
			FisImu_read_reg(FisRegister_Ctrl5, &read_data, 1);
			qst_printf("FisRegister_Ctrl5=0x%x \n", read_data);
			FisImu_read_reg(FisRegister_Ctrl6, &read_data, 1);
			qst_printf("FisRegister_Ctrl6=0x%x \n", read_data);
			FisImu_read_reg(FisRegister_Ctrl7, &read_data, 1);
			qst_printf("FisRegister_Ctrl7=0x%x \n", read_data);
		}
		FisImu_set_layout(2);
	}
	else
