...
int vd_xy_2D = 0;
int rd_2D = 0;
static int lastStatus_2D = ACCURACY_UNRELIABLE;		//ACCURACY_HIGH;
static int curAccuracy_2D = ACCURACY_UNRELIABLE;	//ACCURACY_HIGH;
...
//	rok = getCalibrationFromFile(&seedCal2D, wkCoefficients);
	...
	if (rok == 1) {
		...
		//kal_prompt_trace(MOD_MATV,"Load calibration data success !");
...
int checkAccuracy_2D(int curMag_2D, int lastStatus_2D)
{
	...
	if(st_2D == ACCURACY_UNRELIABLE)
	{
		...
		nDiffAccuracy_2D =0;
	//kal_prompt_trace(MOD_MATV,"checkAccuracy2D, st_2D = %d, lastStatus_2D = %d,nDiffAccuracy_2D = %d", st_2D, lastStatus_2D,nDiffAccuracy_2D);
	return st_2D;
}
...
int process(REAL *mMagneticData)
{
	...
	if(Is2DReady ==1)
	{
		if(bEnable_ELLIPSE_2D)
		{
			...
			int MagR_2D;
#if 0			
			memset(buf_qbg_double,0,100);
			sprintf(buf_qbg_double,"ELLIPSE_2D [%f, %f, %f, %f, %f, %f, %f]\n",
					M_CosT,M_SinT,M_E_2D_ofsX,M_E_2D_ofsY,M_R1,M_R2,M_T); 															
			//kal_prompt_trace(MOD_MATV,buf_qbg_double);
#endif 
